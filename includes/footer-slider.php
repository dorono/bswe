...
if('' == themify_get('setting-footer_slider_enabled') || 'on' == themify_get('setting-footer_slider_enabled')) { ?>
	...
	<div id="footer-slider" class="pagewidth slider">
		...
		<ul class="slides clearfix">
			...
			$img_width = themify_check('setting-footer_slider_width')?	themify_get('setting-footer_slider_width'): '220';
			$img_height = themify_check('setting-footer_slider_height')? themify_get('setting-footer_slider_height'): '160';
			
			if(themify_check('setting-footer_slider_posts_category')){
				$cat = "&cat=".themify_get('setting-footer_slider_posts_category');	
			} else {
				$cat = "";
			}
			if(themify_check('setting-footer_slider_posts_slides')){
				$num_posts = "showposts=".themify_get('setting-footer_slider_posts_slides')."&";
			} else {
				$num_posts = "showposts=7&";
			}
			if(themify_check('setting-footer_slider_display') && themify_get('setting-footer_slider_display') == "images"){ 
				...
				if( have_posts() ) {
					...
					while ( have_posts() ) : the_post(); ?>
						...
						<div class="slide-content-wrap">
							...
							<?php if(themify_get('setting-footer_slider_hide_title') != 'yes'): ?>
								<h3 class="slide-post-title"><a href="<?php echo $link; ?>" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h3>
							<?php endif; ?>
							
							<?php if(themify_get('setting-footer_slider_default_display') == 'content'): ?>
								<div class="slide-excerpt">
								<?php the_content(); ?>
								</div>
							<?php elseif( ! themify_get('setting-footer_slider_default_display') || themify_get('setting-footer_slider_default_display') == 'none'): ?>
									<?php //none ?>
							<?php else: ?>
								<div class="slide-excerpt">
								<?php the_excerpt(); ?>
