...
/***************************************************************************/
			...
			$output .= '</select>
					...
					<p>
						...
						foreach($wrap_options as $name => $value){
							if ( isset( $data['setting-header_slider_wrap'] ) && ( $data['setting-header_slider_wrap'] == $value ) ) {
	...
	function themify_footer_slider($data=array()){
		...
		if ( ! isset( $data['setting-footer_slider_visible'] ) || '' == $data['setting-footer_slider_visible'] ) {
			$data['setting-footer_slider_visible'] = "4";	
		}
		
		if ( 'images' == themify_get( 'setting-footer_slider_display' ) ) {
			$display_images = "checked='checked'";
			$display_posts_display = "style='display:none;'";
		} else {
			$display_posts = "checked='checked'";	
		 ...
		 * @var string
		 */
		$output = '<p><span class="label">' . __('Enable Slider', 'themify') . '</span> <select name="setting-footer_slider_enabled" class="feature_box_enabled_check">';
		/** Iterate through slider options */
		foreach ( $slider_ops as $key => $val ) {
			$output .= '<option value="'.$val.'" ' . selected( themify_get( 'setting-footer_slider_enabled' ), $val, false ) . '>' . $key . '</option>';
		}
		$output .= '</select>' . '</p>
					...
					<p class="pushlabel feature_box_posts" '.$display_posts_display.'>';
							$output .= wp_dropdown_categories(array("show_option_all"=>__('All Categories', 'themify'),"hide_empty"=>0,"echo"=>0,"name"=>"setting-footer_slider_posts_category","selected"=> themify_get( 'setting-footer_slider_posts_category' )));
		$output .=	'	<input type="text" name="setting-footer_slider_posts_slides" value="' . themify_get( 'setting-footer_slider_posts_slides' ) . '" class="width2" /> ' . __('number of posts to be queried', 'themify') . ' 
					</p>
		...
		$output .= '
			...
			$output .= '<div class="feature_box_images" '.$display_images_display.'>';
					...
					foreach ( $image_options as $option ) {
						$x++;
			$output .= '<div class="uploader-fields">
							<span class="label">' . __('Title', 'themify') . ' '.$x.'</span>
							<input type="text" class="width10" name="setting-footer_slider_images_'.$option.'_title" value="' . themify_get( 'setting-footer_slider_images_'.$option.'_title' ) . '" />
							<span class="label">' . __('Link', 'themify') . ' '.$x.'</span>
							<input type="text" class="width10" name="setting-footer_slider_images_'.$option.'_link" value="' . themify_get( 'setting-footer_slider_images_'.$option.'_link' ) . '" />
							<span class="label">' . __('Image', 'themify') . ' '.$x.'</span>
							<input type="text" class="width10 feature_box_img" id="setting-footer_slider_images_'.$option.'_image" name="setting-footer_slider_images_'.$option.'_image" value="' . themify_get( 'setting-footer_slider_images_'.$option.'_image' ) . '" /> 
							<div class="pushlabel" style="display:block;">' . themify_get_uploader('setting-footer_slider_images_'.$option.'_image', array('tomedia' => true)) . '</div>
						</div>';
					}
		...
		$output .= '</div>
			...
			$output .= '
					...
					<span class="label">' . __('Scroll', 'themify') . '</span>
								...
								';
							foreach($scroll_options as $option){
								if ( isset( $data['setting-footer_slider_scroll'] ) && ( $option == $data['setting-footer_slider_scroll'] ) ) {
									$output .= '<option value="'.$option.'" selected="selected">'.$option.'</option>';
								} else {
									$output .= '<option value="'.$option.'">'.$option.'</option>';
								}
			...
			$output .=	'</select>
					...
					<p>
						<span class="label">' . __('Feature Image Size', 'themify') . '</span>
						<input type="text" name="setting-footer_slider_width" class="width2" value="' . themify_get( 'setting-footer_slider_width' ) . '" /> ' . __('width', 'themify') . ' <small>(in px)</small>
						<input type="text" name="setting-footer_slider_height" class="width2" value="' . themify_get( 'setting-footer_slider_height' ) . '" /> ' . __('height', 'themify') . ' <small>(in px)</small>
					</p>
					</div>';		
		return $output;
	}
	...
	function themify_default_page_layout($data=array()){
		...
		$options = array(
			array('value' => 'sidebar2', 'img' => 'images/layout-icons/sidebar2.png', 'selected' => true, 'title' => __('Left and Right', 'themify')),
